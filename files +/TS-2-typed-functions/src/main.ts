...
{
  type multiplication = (a: number, b: number) => number;

  const multiply: multiplication = (a, b) => {
    return a * b;
  };

  type powerValue = (base: number, power: number) => number;

  const power: powerValue = function (base, power) {
    return base ** power;
  };

  type rootFunction = (base: number, nthRoot: number) => number;
  ...
  type blueishing = (text: string);

  const printBlueText: blueishing = (text) => console.log(`%c${text}`, 'color: #0000ee');
